...
class com_okeydocInstallerScript
{
  ...
  function postflight($type, $parent) 
  {
    if($type == 'install') {
      ...
      //Informations about the Okey DOC 2 document items.
      $columns = array('type_title', 'type_alias', $db->quoteName('table'), 'field_mappings', 'router');
      $query->clear();
      $query->insert('#__content_types');
      $query->columns($columns);
      $query->values($db->Quote('Okey DOC 2').','.$db->Quote('com_okeydoc.document').','.
...
$db->Quote('OkeydocHelperRoute::getDocumentRoute'));
      ...
      //Informations about the Okey DOC 2 category items.
      $query->clear();
      $query->insert('#__content_types');
      $query->columns($columns);
      $query->values($db->Quote('Okey DOC 2 Category').','.$db->Quote('com_okeydoc.category').','.
